...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
 ...
 useEffect(() => {
  ...
  const value: AppContextType = {
    ...
    playLuckyDraw: () => api.playLuckyDraw().then(r => { setCurrentUser(r.user || null); return { success: true, prize: r.prize }; }).catch(e => { addNotification(e.message, 'error'); return { success: false }; }),
    ...
    performDailyCheckIn: () => api.performDailyCheckIn().then(r => { setCurrentUser(r.user || null); addNotification(r.message, 'success'); return r; }).catch(e => { addNotification(e.message, 'error'); return { success: false, message: e.message, reward: 0 }; }),
